...
int main(int argc, char* argv[])
{
    ...
    //automatically select the camera - Linux version
    ...
    while (1) {
        ...
        if (key == 'q')
            ...
            snprintf(number, sizeof(number), "_%03d", count);
...
void check_Connected_Camera(int* idx_l, int* idx_r) {
    // Linux version - simplified camera detection
    // In Linux, cameras are typically /dev/video0, /dev/video1, etc.
    cout << "Available cameras:" << endl;
    
    // Try to find available cameras
    for (int i = 0; i < 10; i++) {
        VideoCapture test_cam(i);
        if (test_cam.isOpened()) {
            cout << "Camera " << i << " is available" << endl;
            test_cam.release();
        }
    }
    
    cout << "Please enter left camera index: ";
    cin >> *idx_l;
    cout << "Please enter right camera index: ";
    cin >> *idx_r;
